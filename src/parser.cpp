...
void printToken(Token token){
    switch (token) {
        ...
        case tok_extern: printf("extern"); break;
        ...
        case tok_switch: printf("switch"); break;
        case tok_case: printf("case"); break;
        case tok_default: printf("default"); break;
        case tok_import: printf("import"); break;
        case tok_const: printf("const"); break;
        case tok_id: printf("identifier"); break;
        case tok_fnumber: printf("float number"); break;
        case tok_inumber: printf("int number"); break;
        case tok_true: printf("true"); break;
        case tok_false: printf("false"); break;
        case tok_charlit: printf("char literal"); break;
        case tok_literal: printf("string literal"); break;
        case tok_void: printf("void"); break;
        case tok_bool: printf("bool"); break;
        case tok_char: printf("char"); break;
        case tok_uchar: printf("uchar"); break;
        case tok_short: printf("short"); break;
        case tok_ushort: printf("ushort"); break;
        case tok_int: printf("int"); break;
        case tok_uint: printf("uint"); break;
        case tok_long: printf("long"); break;
        case tok_ulong: printf("ulong"); break;
        case tok_float: printf("float"); break;
        case tok_double: printf("double"); break;
        ...
        case tok_rh: printf(">>"); break;
        case tok_urh: printf(">>>"); break;
        case tok_lh: printf("<<"); break;
        case tok_ulh: printf("<<<"); break;
        case tok_or: printf("||"); break;
        case tok_and: printf("&&"); break;
        case tok_bitor: printf("|"); break;
        case tok_bitand: printf("&"); break;
        case tok_bitxor: printf("^"); break;
        case tok_ing: printf("ing"); break;
...
static std::map<std::string, Token> KeyWordTkMap = {
    {"recipe", tok_def}, {"extern", tok_extern}, {"taste", tok_if}, {"bake", tok_for}, {"stir", tok_while},
    {"swallow", tok_else}, {"then", tok_then}, {"in", tok_in}, {"bill", tok_return}, {"nextPls", tok_continue},
    {"giveUp", tok_break}, {"menu", tok_struct}, {"favorite", tok_switch}, {"case", tok_case}, {"otherwise", tok_default},
    {"true", tok_true}, {"false", tok_false}, {"void", tok_void}, {"bool", tok_bool}, {"char", tok_char}, {"u8", tok_uchar},
    {"i16", tok_short}, {"u16", tok_ushort}, {"i32", tok_int}, {"u32", tok_uint}, {"i64", tok_long},
    {"u64", tok_ulong}, {"f32", tok_float}, {"f64", tok_double}, {"import", tok_import}, {"freeze", tok_const}, {"ing", tok_ing}
};
...
DataDeclAST *GrammarParser::parseVarDef() {
    ...
    if(TkParser->lookUp(1)[0] != tok_ing) {
        printToken(TkParser->lookUp(1)[0]);
        LOG_ERROR("ing expected", TkParser->getCurLineNo());
    }
    getNextToken();
    ...
    while(TkParser->lookUp(1)[0] != ';') {
        ...
        if(TkParser->lookUp(1)[0] != ':') {
          LOG_ERROR("expected a : after varibale name", line)
        }
        getNextToken();
        DataTypeAST *type = parseTypeDecl();
        var->setDataType(type);
        // printToken(TkParser->lookUp(1)[0]);
        // printf("%d", TkParser->lookUp(1)[0] == '[');
...
ParamAST *GrammarParser::parseParamDecl()   {
    ...
    getNextToken();
    if(TkParser->lookUp(1)[0] != ':') {
      LOG_ERROR("expected ':' colon after variable name", TkParser->getCurLineNo());
    }
    getNextToken();
    // eat type
