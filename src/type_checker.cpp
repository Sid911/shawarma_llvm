...
namespace shw {
    ...
    bool TypeChecker::isConstant(ExprAST *node) {
        switch (node->getClassId()) {
            ...
            case UnaryExprId: return isConstant(shw_cast<UnaryExprAST>(node)->getUnaryExpr());
            case BinExprId: {
                BinaryExprAST *bin = shw_cast<BinaryExprAST>(node);
    ...
    void TypeChecker::visit(shw::IdRefAST *node) {
        node->setExprType(node->getId()->getDataType()->getDataType());
        node->setIsSigned(isSigned(node->getExprType()));
    }

    void TypeChecker::visit(shw::IdIndexedRefAST *node) {
        node->setExprType(node->getId()->getDataType()->getDataType());
        node->setIsSigned(isSigned(node->getExprType()));
        AstVisitor::visit(node);
    }

    void TypeChecker::visit(shw::CallExprAST *node) {
        if(node->isCallStd()) {
            if(node->getName() == "serve") {
                node->setExprType(Void);
                node->setIsSigned(false);
            }
            else if(node->getName() == "serveLn") {
                node->setExprType(Void);
                node->setIsSigned(false);
            }
    ...
    void TypeChecker::visit(shw::NumberExprAST *node) {
        // do nothing
