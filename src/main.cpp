...
int parseCmdArgs(int argc, char *argv[]) {
    ...
    try {
        std::unique_ptr<cxxopts::Options> allocated(new cxxopts::Options(argv[0], "shawarma - Shawarma compiler"));
...
int main(int argc, char *argv[]) {
    ...
    ShwIRTypeSupport::initIRTypeSupport();
    ShwIRConstantValueSupport::initIRConastantSupport();
    ShwIRBuilder::initStdFunctionTypeMap();
    /// generate ir
    for(auto *prog : ProgramList) {
        auto builder = ShwIRBuilder::getOrCreateIrBuilderByProg(prog);
        prog->accept(*builder);
...
//        }
    ...
    if(PrintIR) {
        for(auto *prog : ProgramList) {
            auto m = ShwIRBuilder::getOrCreateIrBuilderByProg(prog)->getLLVMModule();
...
#else
     Translate into C code and then compile into an executable file
#endif
