    ...
    if(node->getExprOp() == Assign) {
        ...
        switch (node->getExprOp()) {
            ...
            case Or: {
                ...
                LastValue = TheIRBuilder->CreateZExtOrBitCast(LastValue, ShwIRTypeSupport::ShwBoolType);
                break;
            }
            case And: {
    ...
    switch (node->getExprOp()) {
        case Not: {
            ...
            if(!LastValue->getType()->isIntegerTy(1))
                LastValue = TheIRBuilder->CreateZExt(LastValue, ShwIRTypeSupport::ShwBoolType);
...
void ShwIRBuilder::visit(IdIndexedRefAST *node) {
    ...
    while(begin != end) {
        ...
        if(v) {
            LastValue = TheIRBuilder->CreateNSWMul(LastValue, llvm::ConstantInt::get(ShwIRTypeSupport::ShwIntType, index));
    ...
    LastValue = TheIRBuilder->CreateInBoundsGEP(var->getVarLLVMType(), node->getLLVMValue(), {ShwIRConstantValueSupport::ShwIntZero, v});
    if(!IsNeedPointer) {
...
llvm::Type *ShwIRBuilder::getLLVMType(DataTypeAST *datatype) {
    ...
    {
        case Void: return ShwIRTypeSupport::ShwVoidType;
        case Double: return ShwIRTypeSupport::ShwDoubleType;
        case Float: return ShwIRTypeSupport::ShwFloatType;
        case Bool: return ShwIRTypeSupport::ShwBoolType;
        case Char: return ShwIRTypeSupport::ShwCharType;
        case UChar: return ShwIRTypeSupport::ShwUCharType;
        case Short: return ShwIRTypeSupport::ShwShortType;
        case UShort: return ShwIRTypeSupport::ShwUShortType;
        case Int: return ShwIRTypeSupport::ShwIntType;
        case Uint: return ShwIRTypeSupport::ShwUIntType;
        case Long: return ShwIRTypeSupport::ShwLongType;
        case ULong: return ShwIRTypeSupport::ShwULongType;
        case Enum:
        case Struct:
        case Pointer:
...
void ShwIRBuilder::createAndSetCurrentFunc(const llvm::StringRef& name, llvm::FunctionType *ty) {
...
void ShwIRBuilder::storeValueToPointer(llvm::Type *lhs, llvm::Value *lv, llvm::Value *rv) {
...
void ShwIRBuilder::generateStdFuncCall(CallExprAST *node) {
    ...
    if(node->getName() == "serve" || node->getName() == "serveLn") {
        for(auto arg : node->getArgs()) {
            arg->accept(*this);
            if(LastValue->getType()->isFloatTy()) {
                LastValue = TheIRBuilder->CreateFPExt(LastValue, ShwIRTypeSupport::ShwDoubleType);
            }
            args.push_back(LastValue);
        }
...
void ShwIRBuilder::convertToI1() {
    ...
    else if(ty->isIntegerTy()) LastValue = TheIRBuilder->CreateTrunc(LastValue, ShwIRTypeSupport::ShwBoolType);
    else { assert(false); }

}

llvm::Type *ShwIRBuilder::kaleTypeToLLVMType(KType ty) {
    switch (ty) {
        case Bool: return ShwIRTypeSupport::ShwBoolType;
        case Char: return ShwIRTypeSupport::ShwCharType;
        case UChar: return ShwIRTypeSupport::ShwUCharType;
...
std::unordered_map<ProgramAST *, ShwIRBuilder *> ShwIRBuilder::ProgToIrBuilderMap = {};
...
std::unordered_map<std::string, std::vector<KType>> ShwIRBuilder::StdKaleFuncTypeMap = {};
std::unordered_map<std::string, llvm::FunctionType*> ShwIRBuilder::StdLLVMFuncTypeMap = {};
...
void ShwIRBuilder::initStdFunctionTypeMap() {
    ...
    ty = llvm::FunctionType::get(ShwIRTypeSupport::ShwVoidType, {llvm::Type::getInt8PtrTy(GlobalContext)}, true);
